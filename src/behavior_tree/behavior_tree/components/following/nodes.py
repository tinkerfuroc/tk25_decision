...
from behavior_tree.components.following.wave_reseed_cycle import (
    WaveReseedCycle,
    _WaveReseedBridge,
)
...
class BtNode_ReacqAnnounce(py_trees.behaviour.Behaviour):
    ...
    def setup(self, **kwargs):
        ...
        from behavior_tree.interfaces.messages import TextToSpeech
...
class BtNode_RecoveryScan(py_trees.behaviour.Behaviour):
    ...
    imports keep ``behavior_tree.interfaces.messages`` off the import path until live setup.
    """
    ...
    def setup(self, **kwargs):
        ...
        if self._pan_sink is None and not self.mock_mode:
            from behavior_tree.interfaces.messages import PanTiltCommand
