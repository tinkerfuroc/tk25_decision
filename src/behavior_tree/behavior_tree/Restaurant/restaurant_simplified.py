...
"""Batch-first Restaurant behavior tree.

Strategy:
1) Detect callers in a batch
2) Collect all orders
3) Return to known bar/start position and call barman
4) Serve all customers with valid orders
"""
...
from .config import (
    ...
    KEY_ARM_NAVIGATING,
    KEY_ARM_SERVING,
    KEY_BATCH_DETECTION_SUMMARY,
    KEY_BATCH_ORDERS_SUMMARY,
...
class BtNode_BuildBatchDetectionSummary(py_trees.behaviour.Behaviour):
    """Build a concise spoken summary from the detected-customer batch."""
...
def _createDetectAndAddOne():
    """Single detection attempt: find one caller and append to batch."""
...
def createPhaseDetectCustomers():
    """Phase A: detect up to `BATCH_SIZE_LIMIT` callers and announce batch summary."""
...
def _createTakeAndConfirmOrder():
    """Reusable order intake subtree for one customer."""
...
def _createCollectOneCustomerOrder():
    """Collect one customer's order or mark failure, then advance index."""
...
def createPhaseCollectOrders():
    """Phase B: iterate the detected batch and collect orders from each customer."""
...
def _createPlaceOneOrder():
    """Place one selected customer's order with barman (if available)."""
...
def createPhasePlaceAllOrders():
    """Phase C: return to known start/bar position and place all collected orders."""
...
def createOptionalTrayTransport():
    """Optional tray branch; falls back to direct transport when tray is unavailable."""
...
def _createServeOneCustomer():
    """Serve one selected customer if they still need service."""
...
def createPhaseServeOrders():
    """Phase D: deliver all unserved valid orders from the batch."""
...
def createRestaurantSimplifiedTask():
    """Compose all simplified phases and guard runtime with a global timeout."""
    ...
    # Timeout returns FAILURE when exceeded; selector fallback announces and exits gracefully.
...
def restaurant_simplified():
    """Runtime entry for the simplified strategy."""
    run_tree(
