...
# server) pops up a real cv2 window with per-person bounding boxes -- nothing
# to launch here for that, it's automatic whenever the server is up.
...
class BtNode_GateBlackBoardList(py_trees.behaviour.Behaviour):
    def __init__(self, name, bb_key_list, n_gate=2):
        super().__init__(name)
        self.blackboard = self.attach_blackboard_client(name=self.name)
        self.blackboard.register_key(
...
def scan_once(pan: float, tilt: float = 40.0, target_frame="base_link"):
    root = py_trees.composites.Sequence(
        name=f"scan at {pan} x for waving customers", memory=True
    )
    root.add_child(
    ...
    root.add_child(
        BtNode_Announce(
            name=f"announce found customer at {pan}",
            bb_source=None,
            message="I see someone waving!",
        )
    )
    root.add_child(
...
def scanAllPositions(target_frame="base_link"):
    root = py_trees.composites.Sequence(
        name="turn pan tilt to look in six positions for calling customer", memory=True
    )
    ...
    for pan in [
        ...
        pre_gate = py_trees.composites.Selector(
            f"gate and then scan for x {pan}", memory=True
        )
        pre_gate.add_child(
