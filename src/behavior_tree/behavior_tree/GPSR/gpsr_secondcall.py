...
pose_sink = possible_poses["sink"]
pose_podium = possible_poses["podium"]
pose_kitchen = possible_poses["kitchen"]
pose_living_room = possible_poses["living_room"]
pose_office = possible_poses["office"]
...
KEY_POSE_SINK = "sink"
KEY_POSE_PODIUM = "podium"
KEY_POSE_KITCHEN = "kitchen"
KEY_POSE_LIVING_ROOM = "living_room"
KEY_POSE_OFFICE = "office"
...
def createConstantWriter():
    ...
    root.add_child(BtNode_WriteToBlackboard("Write sink pose", bb_namespace="", bb_source=None, bb_key=KEY_POSE_SINK, object=pose_sink))
    root.add_child(BtNode_WriteToBlackboard("Write podium pose", bb_namespace="", bb_source=None, bb_key=KEY_POSE_PODIUM, object=pose_podium))
    ...
    root.add_child(BtNode_WriteToBlackboard("Write living room pose", bb_namespace="", bb_source=None, bb_key=KEY_POSE_LIVING_ROOM, object=pose_living_room))
    root.add_child(BtNode_WriteToBlackboard("Write office pose", bb_namespace="", bb_source=None, bb_key=KEY_POSE_OFFICE, object=pose_office))
...
def createGPSR():
    ...
    command2 = "take the waving person from the podium to the kitchen"
    ...
    execution1.add_child(BtNode_GotoAction("go to sink", KEY_POSE_SINK))
    ...
    execution1.add_child(BtNode_Announce(name="announce smallest object", bb_source=None, message="The smallest object on the sink is on the left, it is a cola can."))
    root.add_child(execution1)

    execution2 = py_trees.composites.Sequence("execute second command", True)
    execution2.add_child(BtNode_Announce(name="announce confirmed", bb_source=None, message="The execution procedure for this command is go to the podium, scan for the waving person, announce follow me, go to the office. Starting execution."))
    execution2.add_child(BtNode_GotoAction("go to desk lamp", KEY_POSE_PODIUM))
    ...
    execution2.add_child(BtNode_GotoAction("go to kitchen", KEY_POSE_OFFICE))
    execution2.add_child(BtNode_Announce(name="announce arrived kitchen", bb_source=None, message="We have arrived at the office. You can stop following me now."))
    root.add_child(execution2)

    execution3 = py_trees.composites.Sequence("execute third command", True)
    execution3.add_child(BtNode_Announce(name="announce confirmed", bb_source=None, message="The execution procedure for this command is to go to the living room, scan for the raising person, go to the raising person, announce ask the question, announce the answer. Starting execution."))
    execution3.add_child(BtNode_GotoAction("go to living room", KEY_POSE_LIVING_ROOM))
