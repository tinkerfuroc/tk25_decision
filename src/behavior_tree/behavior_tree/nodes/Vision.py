...
from behavior_tree.interfaces.vision import ObjectDetection, ObjectDetectionGeneralist, Object, FeatureExtraction, FeatureExtractionAction, SeatRecommendation, SeatRecommendationAction, FeatureMatching, FeatureMatchingAction, GetImage, GetPointCloud, DoorDetection, PanTiltCtrl, BoundingBox, PanTiltCommand, PanTiltState, FollowHeadAction, SeatRecommendBboxAction, DetectWavingAction, PlacingLocation, ObjectScanAction
...
class BtNode_FindObj(ServiceHandler):
    ...
    def initialise(self) -> None:
        ...
        # Handle mock mode
        if self.mock_mode:
            from behavior_tree.interfaces.messages import ObjectDetection
...
class BtNode_GetImage(ServiceHandler):
    """Capture RGB and optional depth data from a named camera."""
    ...
    def update(self):
        ...
        if not self.response.done():
            self.feedback_message = (
                f"Waiting for image response from {self.camera}"
            )
            return pytree.common.Status.RUNNING
        response = self.response.result()
        if response.status != 0:
            self.feedback_message = (
                f"GetImage failed with status {response.status}: "
        ...
        if self.get_depth_image:
            try:
                ...
                pass
        self.feedback_message = f"Captured image from {self.camera}"
        return pytree.common.Status.SUCCESS
