...
from behavior_tree.interfaces.manipulation import (
    Grasp,
    Drop,
    Place,
    JointMove,
...
from behavior_tree.core.constants import SCAN_POSES
    ...
    def process_result(self):
        if self.result_status != action_msgs.GoalStatus.STATUS_SUCCEEDED:
            ...
            self.feedback_message = (
                f"CartesianMove action_status={self.result_status}: "
...
class BtNode_PointTo(ActionHandler):
    ...
    def __init__(
        ...
        *,
        ...
        # and nodes.pointing_math.compute_point_to_pan).
...
class BtNode_ScanAndPlace(ActionHandler):
    ...
    def process_result(self):
        from behavior_tree.core.events import record_event
...
class BtNode_JointMoveAction(ActionHandler):
    ...
    def process_result(self):
        if self.result_status != action_msgs.GoalStatus.STATUS_SUCCEEDED:
            ...
            err = str(getattr(result, "error_msg", "missing error detail"))
            self.feedback_message = (
                f"JointMoveAction action_status={self.result_status}: "
                f"result(status={st}, stage={sg}, error={err})"
            )
            self.logger.debug(f"MoveArmJointPC failed: status={st}, stage={sg}, error={err}")
            return pytree.common.Status.FAILURE
        else:
            result = self.result_message.result
            legacy_success = bool(getattr(result, "success", True))
