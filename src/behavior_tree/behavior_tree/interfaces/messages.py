"""
Message imports for behavior_tree package.
Conditionally imports real or mock messages based on availability.
"""
from behavior_tree.core.config import get_config

# Get configuration
_config = get_config()

# Import based on availability
if _config.has_dependency('tinker_vision_msgs_26'):
    from tinker_vision_msgs_26.srv import ObjectDetection, FeatureExtraction, SeatRecommendation, FeatureMatching, GetPointCloud, DoorDetection, FollowHead, DetectWaving
    from tinker_vision_msgs_26.srv import ObjectDetectionGeneralist, SeatRecommendBbox, PlacingLocation, GetImage, ReseedTarget, ObjectScan
    from tinker_vision_msgs_26.msg import Object, PanTiltCtrl, PanTiltCommand, PanTiltState, BoundingBox
    from tinker_vision_msgs_26.action import (
        Categorize,
        DetectWaving as DetectWavingAction,
        FeatureExtraction as FeatureExtractionAction,
        FeatureMatching as FeatureMatchingAction,
        FollowHeadAction,
        HumanFollowing,
        ObjectScan as ObjectScanAction,
        SeatRecommendBbox as SeatRecommendBboxAction,
        SeatRecommendation as SeatRecommendationAction,
        TrackPerson,
    )
else:
    from behavior_tree.interfaces.mock_messages import ObjectDetection, FeatureExtraction, SeatRecommendation, FeatureMatching, GetPointCloud, DoorDetection, FollowHead, DetectWaving
    from behavior_tree.interfaces.mock_messages import (
        GetImage,
        ObjectDetectionGeneralist,
        ObjectScan,
        PlacingLocation,
        ReseedTarget,
        SeatRecommendBbox,
    )
    from behavior_tree.interfaces.mock_messages import Object, PanTiltCtrl, PanTiltCommand, PanTiltState, BoundingBox
    from behavior_tree.interfaces.mock_messages import (
        Categorize,
        DetectWavingAction,
        FeatureExtractionAction,
        FeatureMatchingAction,
        FollowHeadAction,
        HumanFollowing,
        ObjectScanAction,
        SeatRecommendBboxAction,
        SeatRecommendationAction,
        TrackPerson,
    )

if _config.has_dependency('tinker_arm_msgs'):
    from tinker_arm_msgs.srv import Drop, ArmJointService, PointTo
    from tinker_arm_msgs.action import Place, Grasp, JointMove, CartesianMove, ScanAndPlace
    try:
        from tinker_arm_msgs.action import Fold
    except ImportError:
        from behavior_tree.interfaces.mock_messages import Fold
else:
    from behavior_tree.interfaces.mock_messages import Drop, ArmJointService, PointTo
    from behavior_tree.interfaces.mock_messages import Place, Grasp, JointMove, CartesianMove, ScanAndPlace
    from behavior_tree.interfaces.mock_messages import Fold

if _config.has_dependency('tinker_audio_msgs'):
    try:
        from tinker_audio_msgs.srv import TTSCnRequest, TextToSpeech, WaitForStart, PhraseExtraction, GetConfirmation, Listen, CompareInterest, QuestionAnswer, GraspRequest
    except ImportError:
        # tinker_audio_msgs present but missing a symbol (TTSCnRequest hazard) ->
        # fall back to the mock srvs. Does NOT reproduce on this host; harmless.
        from behavior_tree.interfaces.mock_messages import TTSCnRequest, TextToSpeech, WaitForStart, PhraseExtraction, GetConfirmation, Listen, CompareInterest, QuestionAnswer, GraspRequest
    from tinker_audio_msgs.action import GetConfirmation as GetConfirmationAction, Listen as ListenAction
    from tinker_audio_msgs.action import PhraseExtraction as PhraseExtractionAction
    from tinker_audio_msgs.action import NameDrinkExtraction as NameDrinkExtractionAction
    from tinker_audio_msgs.action import Doorbell
    from tinker_audio_msgs.action import NameDrinkExtraction as NameDrinkExtractionAction
    try:
        from tinker_audio_msgs.action import OrderExtraction as OrderExtractionAction
    except ImportError:
        # tinker_audio_msgs present but does not export the OrderExtraction
        # action (older build on this host) -> use the mock stub so importing
        # behavior_tree still succeeds. Only Restaurant/order flows use it.
        from behavior_tree.interfaces.mock_messages import OrderExtractionAction
else:
    from behavior_tree.interfaces.mock_messages import TTSCnRequest, TextToSpeech, WaitForStart, PhraseExtraction, GetConfirmation, Listen, CompareInterest, QuestionAnswer, GraspRequest
    from behavior_tree.interfaces.mock_messages import GetConfirmationAction, ListenAction
    from behavior_tree.interfaces.mock_messages import PhraseExtractionAction
    from behavior_tree.interfaces.mock_messages import NameDrinkExtractionAction
    from behavior_tree.interfaces.mock_messages import Doorbell
    from behavior_tree.interfaces.mock_messages import NameDrinkExtractionAction
    from behavior_tree.interfaces.mock_messages import OrderExtractionAction

if _config.has_dependency('tinker_nav_msgs'):
    from tinker_nav_msgs.srv import SetLuggagePose, ComputeGrasp, OrientationAngle
    # FindApproachPose grew a `projection_mode` field (P3); fall back to the
    # mock stub if an older tinker_nav_msgs without the srv is sourced.
    try:
        from tinker_nav_msgs.srv import FindApproachPose
    except (ImportError, ModuleNotFoundError):
        from behavior_tree.interfaces.mock_messages import FindApproachPose
    try:
        from tinker_nav_msgs.action import GoToApproach
    except (ImportError, ModuleNotFoundError):
        from behavior_tree.interfaces.mock_messages import GoToApproach
    try:
        from tinker_nav_msgs.action import Backward
    except (ImportError, ModuleNotFoundError):
        from behavior_tree.interfaces.mock_messages import Backward
else:
    from behavior_tree.interfaces.mock_messages import (
        Backward,
        ComputeGrasp,
        FindApproachPose,
        GoToApproach,
        OrientationAngle,
        SetLuggagePose,
    )

# `Follow` lives only in tk26_ws's tinker_nav_msgs; the tk25_ws package of the
# same name ships srv-only. Import defensively so BT builds even when only
# tk25 is sourced — the mock stub still exercises the tree shape.
try:
    from tinker_nav_msgs.action import Follow
except (ImportError, ModuleNotFoundError):
    from behavior_tree.interfaces.mock_messages import Follow

if _config.has_dependency('nav2_msgs'):
    from nav2_msgs.action import NavigateToPose
    print("Using real nav2_msgs NavigateToPose")
else:
    from behavior_tree.interfaces.mock_messages import NavigateToPose
    print("WARNING: nav2_msgs not found, using mock NavigateToPose !!!")

if _config.has_dependency('control_msgs'):
    from control_msgs.action import GripperCommand
    from control_msgs.msg import JointJog
else:
    from behavior_tree.interfaces.mock_messages import GripperCommand, JointJog

if _config.has_dependency('action_msgs'):
    import action_msgs.msg as action_msgs
else:
    from behavior_tree.interfaces.mock_messages import MockActionMsgsModule as action_msgs

# Standard ROS2 imports (always available)
import rclpy
from geometry_msgs.msg import Point, PointStamped, Pose, PoseStamped, Quaternion
from std_msgs.msg import Header

# Print status on import
if _config.is_mock_mode():
    print("\n" + "="*60)
    print("⚠️  Behavior Tree: Running with Mock Messages")
    print("="*60)
    _config.print_status()
    print()
